...
class DailyStatRepositoryTest extends Specification {
    ...
    // naverclient 사용하지 않는데 오류 발생하기 때문에
    // Mocking 처리
    @SpringBean
    ...
    def "저장 후 조회가 된다."() {
        ...
        when:
        def dailStat = new DailyStat(givenQuery, LocalDateTime.now())
        def saved = dailyStatRepository.saveAndFlush(dailStat)

        then: "실제 저장이 된다."
        saved.id != null

        when: "Entity Manger를 Clear 하고 재조회한다."
        entityManager.clear()
        def result = dailyStatRepository.findById(saved.id)

        then: "캐시가 아닌 실제 DB쿼리로 데이터를 가져온다."
        verifyAll (result) {
            result.isPresent()
    ...
    def "쿼리의 카운터를 조회한다."() {
        ...
        dailyStatRepository.saveAll(List.of(stat1, stat2, stat3, stat4))
    ...
    def "가장 많이 검색된 쿼리 키워드를 개수와 함께 상위 5개 반환한다."() {
        given:
        ...
        def r = dailyStatRepository.saveAll(List.of(stat1, stat2, stat3, stat4, stat5, stat6, stat7, stat8, stat9, stat10))
