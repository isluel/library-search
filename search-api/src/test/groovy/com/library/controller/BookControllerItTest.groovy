...
class BookControllerItTest extends Specification {
    ...
    def "query가 비어있을 경우 BadRequest 응답이 반환된다."() {
        given:
    ...
    def "page가 음수일 경우 BadRequest 응답이 반환된다."() {
        given:
        def request = new SearchRequest(query: "TST", page: -1, size: 10)
    ...
    def "size가 50을 초과할 경우 BadRequest 응답이 반환된다."() {
        given:
        def request = new SearchRequest(query: "TST", page: 1, size: 52)
