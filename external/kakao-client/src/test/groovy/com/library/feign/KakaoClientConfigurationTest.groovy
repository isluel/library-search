...
class KakaoClientConfigurationTest extends Specification {
    KakaoClientConfiguration kakaoConfiguration;

    void setup() {
        kakaoConfiguration = new KakaoClientConfiguration()
    }

    def "requestInterceptor 의 header에 key 값이 적용된다."() {
        given:
        ...
        def interceptor = kakaoConfiguration.kakaoRequestInterceptor(restApiKey)
        interceptor.apply(template)

        then: "interceptor 이후에는 header 가 추가된다."
